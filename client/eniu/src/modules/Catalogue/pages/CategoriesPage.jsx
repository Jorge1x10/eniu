...
export default function CategoriesPage() {
  ...
  const loadData = useCallback(async (signal) => {
    ...
    if (!categoryResponse.ok) {
      setLoadError(getCategoryErrorMessage(categoryResponse, t("No pudimos cargar las categorías. Intenta nuevamente.")));
      setIsLoading(false);
      return;
    }
  ...
  function savedCategory(category, wasEditing) {
    ...
    setEditing(null);
    setIsCreating(false);
    setSuccess(wasEditing ? t("La categoría se actualizó correctamente.") : t("La categoría se creó correctamente."));
  }

  function deletedCategory(categoryId) {
    setCategories((current) => current.filter((item) => item.id !== categoryId));
    setDeleting(null);
    setSuccess(t("La categoría se eliminó. Sus productos ahora están sin categoría."));
  }
  ...
  return (
    <section className="mx-auto w-full max-w-7xl space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <Link to={`${cataloguePath}/products`} className="cursor-pointer rounded-xl border border-[#D9D9D9] px-4 py-2 text-sm font-semibold hover:bg-white">{t("Ver productos")}</Link>
      </div>
      <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-sm font-semibold text-[#8A7420]">{catalogue?.name || t("Menú")}</p>
          <h1 className="mt-1 text-3xl font-bold text-[#111111]">{t("Categorías")}</h1>
          <p className="mt-1 text-sm text-[#666666]">{t("Organiza los productos de este menú.")}</p>
        </div>
        <button type="button" onClick={() => setIsCreating(true)} disabled={isLoading || Boolean(loadError)} className="inline-flex cursor-pointer items-center justify-center gap-2 rounded-xl bg-[#FFE05A] px-5 py-2.5 font-semibold hover:bg-[#E8C93D] disabled:cursor-not-allowed disabled:opacity-50"><Plus size={18} /> {t("Crear categoría")}</button>
      </header>
      {success && <p role="status" className="rounded-xl bg-green-50 px-4 py-3 text-sm text-green-700">{success}</p>}

      {isLoading ? <LoadingGrid label={t("Cargando categorías")} /> : loadError ? (
        <ErrorState message={loadError} onRetry={() => loadData()} />
      ) : categories.length === 0 ? (
        <EmptyState icon={<FolderOpen size={26} />} title={t("Todavía no tienes categorías")} description={t("Crea categorías para organizar los productos de este menú.")} action={t("Crear categoría")} onAction={() => setIsCreating(true)} />
      ) : (
          ...
          {categories.map((category) => (
            <article key={category.id} className="flex min-h-44 flex-col rounded-2xl border border-[#E9DDB7] bg-white p-5 shadow-sm">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0"><h2 className="truncate text-lg font-bold text-[#111111]">{category.name}</h2><span className="mt-2 inline-block rounded-full bg-[#F8E8AE] px-2.5 py-1 text-xs font-semibold text-[#5E501A]">{t("Visible")}</span></div>
              </div>
