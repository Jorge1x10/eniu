...
// Se resuelve en cada uso para que el formato siga al idioma en curso.
...
export default function ProductsPage() {
  ...
  const loadData = useCallback(async (signal) => {
    ...
    if (!productResponse.ok) {
      setLoadError(getProductErrorMessage(productResponse, t("No pudimos cargar los productos. Intenta nuevamente.")));
      setIsLoading(false);
      return;
    }
    if (!categoryResponse.ok) {
      setLoadError(t("No pudimos cargar las categorías de este menú. Intenta nuevamente."));
      setIsLoading(false);
      return;
    }
  ...
  function savedProduct(product, wasEditing) {
    ...
    setEditing(null);
    setIsCreating(false);
    setSuccess(wasEditing ? t("El producto se actualizó correctamente.") : t("El producto se creó correctamente."));
  }

  function deletedProduct(productId) {
    setProducts((current) => current.filter((item) => item.id !== productId));
    setDeleting(null);
    setSuccess(t("El producto se eliminó correctamente."));
  }
  ...
  return (
    <section className="mx-auto w-full max-w-7xl space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <Link to={`${cataloguePath}/categories`} className="cursor-pointer rounded-xl border border-[#D9D9D9] px-4 py-2 text-sm font-semibold hover:bg-white">{t("Administrar categorías")}</Link>
      </div>
      ...
      {!isLoading && !loadError && products.length > 0 && (
        <div className="flex flex-col gap-2 rounded-2xl border border-[#E9DDB7] bg-white p-4 sm:flex-row sm:items-center">
          <label htmlFor="product-filter" className="text-sm font-semibold text-[#2A2A2A]">{t("Filtrar por categoría")}</label>
          <select id="product-filter" value={filter} onChange={(event) => setFilter(event.target.value)} className="cursor-pointer rounded-xl border border-[#D9D9D9] bg-[#FFFDF5] px-4 py-2.5 outline-none sm:ml-auto sm:min-w-60">
            <option value={ALL}>{t("Todos")}</option><option value={UNCATEGORIZED}>{t("Sin categoría")}</option>
      ...
      {isLoading ? <LoadingGrid /> : loadError ? <ErrorState message={loadError} onRetry={() => loadData()} /> : products.length === 0 ? (
        ...
        <div className="rounded-2xl border border-dashed border-[#D7C990] bg-[#FFFDF5] p-10 text-center"><h2 className="text-lg font-bold">{t("No hay productos en esta categoría.")}</h2><button type="button" onClick={() => setFilter(ALL)} className="mt-4 cursor-pointer font-semibold text-[#8A7420] underline">{t("Ver todos")}</button></div>
      ) : (
...
function LoadingGrid() {
  const { t } = useTranslation();
 return <div aria-label={t("Cargando productos")} className="grid animate-pulse gap-4 md:grid-cols-2 xl:grid-cols-3">{[0, 1, 2].map((item) => <div key={item} className="h-64 rounded-2xl border border-[#E9DDB7] bg-white" />)}</div>; }
...
function EmptyState({ onAction }) {
  const { t } = useTranslation();
 return <div className="rounded-2xl border border-dashed border-[#D7C990] bg-[#FFFDF5] px-6 py-14 text-center"><span className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-[#FFE05A]"><PackageOpen size={26} /></span><h2 className="mt-5 text-xl font-bold">{t("Todavía no tienes productos")}</h2><p className="mt-2 text-sm text-[#666666]">{t("Crea tu primer producto para comenzar a construir este menú.")}</p><button type="button" onClick={onAction} className="mt-6 inline-flex cursor-pointer items-center gap-2 rounded-xl bg-[#FFE05A] px-5 py-2.5 font-semibold hover:bg-[#E8C93D]"><Plus size={18} /> {t("Crear producto")}</button></div>; }
