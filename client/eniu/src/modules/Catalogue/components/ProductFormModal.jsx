...
export default function ProductFormModal({
  ...
  function validate() {
    ...
    if (!name) next.name = "Escribe el nombre del producto.";
    else if (name.length > 64) next.name = "El nombre no puede superar 64 caracteres.";
    if (price && (!PRICE_PATTERN.test(price) || Number(price) > 99999999.99)) {
      next.price = "Usa un precio válido, no negativo y con máximo dos decimales.";
    }
    setErrors(next);
  ...
  async function handleSubmit(event) {
    ...
    if (!response.ok) {
      setRequestError(getProductErrorMessage(response, "No pudimos guardar el producto."));
      return;
    }
    ...
    <AccessibleModal labelledBy="product-form-title" isBusy={isSubmitting} onClose={onClose} initialFocusRef={nameRef}>
      <div className="flex items-start justify-between gap-4">
        <div className="flex gap-3">
          ...
          <div>
            <h2 id="product-form-title" className="text-xl font-bold text-[#111111]">{isEditing ? "Editar producto" : "Crear producto"}</h2>
            <p className="mt-1 text-sm text-[#666666]">La categoría y el precio son opcionales.</p>
          </div>
        </div>
      ...
      <form onSubmit={handleSubmit} noValidate className="mt-6 space-y-5">
        <Field label="Nombre" id="product-name" error={errors.name}>
          <input ref={nameRef} id="product-name" value={form.name} maxLength={64} disabled={isSubmitting} onChange={(event) => change("name", event.target.value)} aria-invalid={Boolean(errors.name)} aria-describedby={errors.name ? "product-name-error" : undefined} className="form-control" placeholder="Ej. Hamburguesa clásica" />
        </Field>
        <Field label="Descripción (opcional)" id="product-description">
          <textarea id="product-description" value={form.description} rows={3} disabled={isSubmitting} onChange={(event) => change("description", event.target.value)} className="form-control resize-y" placeholder="Carne, queso y vegetales" />
        </Field>
        <div className="grid gap-5 sm:grid-cols-2">
          <Field label="Precio (opcional)" id="product-price" error={errors.price}>
          ...
          <Field label="Categoría" id="product-category">
        ...
        <div>
          <div className="flex items-center justify-between gap-3">
            <span className="text-sm font-semibold text-[#2A2A2A]">Imágenes <span className="font-normal text-[#777777]">(hasta 5)</span></span>
        ...
        {categories.length === 0 && (
          <p className="rounded-xl bg-[#F8E8AE]/60 px-4 py-3 text-sm text-[#5E501A]">
            Este menú todavía no tiene categorías. Puedes crear el producto sin categoría y organizarlo después.{" "}
            <Link to={categoriesPath} className="font-bold underline">Ir a Categorías</Link>
          </p>
        )}
        {isEditing && (
          <label className="flex cursor-pointer items-center gap-3 rounded-xl border border-[#D9D9D9] bg-white px-4 py-3">
            ...
            <span className="text-sm font-semibold text-[#2A2A2A]">Producto disponible</span>
          </label>
        )}
        ...
        <div className="flex justify-end gap-3">
          ...
          <button type="submit" disabled={isSubmitting || !form.name.trim()} className="cursor-pointer rounded-xl bg-[#FFE05A] px-5 py-2.5 font-semibold hover:bg-[#E8C93D] disabled:cursor-not-allowed disabled:opacity-50">{isSubmitting ? "Guardando..." : isEditing ? "Guardar cambios" : "Crear producto"}</button>
        </div>
      </form>
    </AccessibleModal>
  );
  ...
  return (
    <div className={`relative overflow-hidden rounded-xl border-2 bg-[#F8E8AE] ${isDefault ? "border-[#E8C93D]" : "border-transparent"}`}>
      ...
      <div className="absolute inset-x-0 bottom-0 flex items-center justify-between bg-black/65 p-1.5">
        <button type="button" onClick={onDefault} aria-label={isDefault ? "Imagen principal" : "Usar como imagen principal"} className={`cursor-pointer rounded-lg p-1.5 ${isDefault ? "bg-[#FFE05A] text-[#111111]" : "text-white hover:bg-white/20"}`}><Star size={15} fill={isDefault ? "currentColor" : "none"} /></button>
        <button type="button" onClick={onRemove} aria-label="Quitar imagen" className="cursor-pointer rounded-lg p-1.5 text-white hover:bg-red-600"><Trash2 size={15} /></button>
      </div>
      {isDefault && <span className="absolute left-2 top-2 rounded-full bg-[#FFE05A] px-2 py-1 text-[10px] font-bold text-[#111111]">Principal</span>}
    </div>
  );
}
