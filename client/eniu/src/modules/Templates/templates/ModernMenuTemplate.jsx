...
function ModernProductCard({ product, theme }) {
  ...
  return <article data-analytics-product-key={product.tracking_key || undefined} className={`overflow-hidden rounded-2xl border border-black/10 shadow-sm ${product.is_available ? "" : "opacity-65"}`} style={{ backgroundColor: theme.background_color }}>{theme.show_product_images && <div className="h-28 bg-black/5">{image ? <img src={image} alt={t("Imagen de {{name}}", { name: product.name })} className="h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center text-xs opacity-45">{t("Sin imagen")}</div>}</div>}<div className="p-3"><div className="flex items-start justify-between gap-2"><h3 className="text-sm font-extrabold leading-tight">{product.name}</h3><Availability available={product.is_available} /></div>{product.description && <p className="mt-1 line-clamp-2 text-xs opacity-65">{product.description}</p>}<p className="mt-3 font-black">{product.price === null ? "Consultar" : menuCurrency.format(Number(product.price))}</p></div></article>;
}
