...
import { BarChart3, CalendarDays, Clock3, MousePointerClick, RefreshCw, Star } from "lucide-react";
...
export default function AnalitycsPage() {
  ...
  return <section className="mx-auto w-full max-w-7xl space-y-6 pb-8 text-[#111111]"><header><p className="text-sm font-semibold text-[#8A7420]">{t("Rendimiento del menú")}</p><h1 className="mt-1 text-3xl font-black">{t("Analíticas")}</h1><p className="mt-2 text-sm text-[#666666]">{t("Estas métricas representan visitas e interés; no son ventas ni pedidos.")}</p></header>
    ...
    {!loading && !error && data && <><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3"><MetricCard icon={BarChart3} title={t("Visitas al menú")} value={summary.menu_views.value} detail={comparisonText(summary.menu_views)} /><MetricCard icon={MousePointerClick} title={t("Interacciones con productos")} value={summary.product_interactions.value} detail={comparisonText(summary.product_interactions)} /><MetricCard icon={Star} title={t("Producto con mayor interés")} value={summary.top_product?.name || t("Sin datos")} detail={summary.top_product ? t("{{count}} interacciones", { count: summary.top_product.interactions }) : null} /><MetricCard icon={CalendarDays} title={t("Día con más actividad")} value={summary.busiest_day?.date || t("Sin datos")} detail={summary.busiest_day ? t("{{count}} visitas", { count: summary.busiest_day.views }) : null} /><MetricCard icon={Clock3} title={t("Hora con más actividad")} value={summary.busiest_hour?.label || t("Sin datos")} detail={summary.busiest_hour ? t("{{count}} visitas", { count: summary.busiest_hour.views }) : null} /></div>
      ...
      <section className="rounded-2xl border border-[#E9DDB7] bg-white p-5"><div className="flex flex-wrap items-center justify-between gap-2"><h2 className="text-xl font-bold">{t("Evolución de visitas")}</h2><p className="text-xs text-[#666666]">{t("Cada punto es un día del periodo.")}</p></div><div className="mt-4"><VisitsChart points={data.visits_over_time} /></div></section>
