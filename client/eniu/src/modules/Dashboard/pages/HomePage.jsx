...
export default function HomePage() {
  ...
  const load = useCallback(async (signal) => {
    ...
    if (!response.ok) { setError("No pudimos cargar el resumen de tu negocio."); setIsLoading(false); return; }
  ...
  const metricDetails = {
    monthlyViews: rawData?.analytics ? `Menú seleccionado: ${viewModel.selectedCatalogue?.name}` : "Sin información disponible",
    activeMenus: `${viewModel.metrics.draftMenus} en borrador`,
    products: "Sin resumen agregado disponible",
    qrScans: rawData?.analytics ? "Atribución aproximada en el periodo" : "Sin información disponible",
  };
  ...
  if (!selectedBusiness) {
    return <section className="flex min-h-[70vh] items-center justify-center p-4 text-center"><div className="max-w-lg rounded-[18px] border border-[#E9DDB7] bg-[#FFFDF5] p-8"><h1 className="text-2xl font-extrabold">Selecciona un negocio</h1><p className="mt-3 text-sm text-[#666666]">Selecciona un negocio desde la barra lateral o crea uno para comenzar a administrar tus menús.</p><button type="button" onClick={() => setIsCreateBusinessOpen(true)} className="mt-6 min-h-11 cursor-pointer rounded-xl bg-[#FFE05A] px-5 font-bold focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#111111]">Crear negocio</button><CreateBusinessModal isOpen={isCreateBusinessOpen} onClose={() => setIsCreateBusinessOpen(false)} /></div></section>;
  }

  return <section className="min-w-0 space-y-6 overflow-x-hidden p-2 text-[#111111] font-['Manrope',ui-sans-serif,system-ui] sm:p-4">
    <header className="flex flex-col gap-3 rounded-xl bg-white sm:flex-row sm:items-start sm:justify-between sm:gap-5 p-3 items-center justify-center sm:h-20"><div><h1 className="text-[25px] font-extrabold leading-tight">Hola, {visibleName} 👋</h1><p className="mt-1 text-xs text-[#2A2A2A]">Así se están comportando tus menús hoy.</p></div><button type="button" onClick={openCreateMenu} className="inline-flex min-h-11 w-full cursor-pointer items-center justify-center gap-2 rounded-xl bg-[#FFE05A] px-4.5 text-sm font-bold hover:bg-[#E8C93D] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#111111] sm:w-auto"><Plus aria-hidden="true" size={18} /> Crear menú</button></header>
    {error ? <div className="rounded-2xl border border-red-200 bg-white p-8 text-center"><p role="alert" className="font-semibold">No pudimos cargar el resumen de tu negocio.</p><button type="button" onClick={() => load()} className="mt-4 inline-flex min-h-11 cursor-pointer items-center gap-2 rounded-xl bg-[#2A2A2A] px-4 font-bold text-white"><RefreshCw aria-hidden="true" size={17} /> Reintentar</button></div> : <>
