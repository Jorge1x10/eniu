    ...
    <ScrollView contentInsetAdjustmentBehavior="automatic" style={{ flex: 1, backgroundColor: theme.background }} contentContainerStyle={{ padding: 18, paddingTop: topPadding, paddingBottom: 120, gap: 18 }}>
      ...
      {!selectedBusiness ? <EmptyState title="Primero crea un negocio" description="Ve a Inicio para crear el negocio que contendrá tus menús." /> : <>
        ...
        {query.isLoading ? <LoadingState label="Cargando menús…" /> : query.isError ? <ErrorState message="No pudimos cargar tus menús." onRetry={() => query.refetch()} /> : menus.length ? <View style={{ gap: 12 }}>
          ...
          {atMenuLimit ? <PlanNotice message={`Tu plan actual permite ${limits.max_catalogues_per_business === 1 ? 'un menú' : `${limits.max_catalogues_per_business} menús`} por negocio.`} /> : null}
