...
export function ErrorState({ message, onRetry }: { message: string; onRetry?: () => void }) {
  const theme = useEniuTheme();
  return <View style={{ borderWidth: 1, borderColor: theme.border, backgroundColor: theme.surface, borderRadius: 20, borderCurve: 'continuous', padding: 24, gap: 14 }}><Text selectable style={{ color: theme.danger, lineHeight: 20, textAlign: 'center' }}>{message}</Text>{onRetry ? <Button variant="secondary" onPress={onRetry}>Reintentar</Button> : null}</View>;
}
