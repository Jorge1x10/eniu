 ...
 *
 * `src/i18n/i18n.test.ts` lo ejecuta; también sirve suelto:
...
const TEXT_ATTRS = new Set([
  ...
  "description", "legend", "hint", "tooltip", "reason", "message", "action",
  "subtitle", "placeholderText", "emptyMessage",
]);

// Clases de Tailwind, selectores, pilas tipográficas y cabeceras HTTP: se
// parecen a texto, pero nadie las lee.
const NOT_TEXT =
  /\b(bg|text|border|px|py|pt|pb|pl|pr|mt|mb|ml|mr|w|h|min-h|max-h|gap|rounded|flex|grid|inline-flex|cursor|overflow|shrink|opacity|ring|z|inset|space-y|space-x|object|accent|justify|items|absolute|relative|fixed|sticky|block|hidden|truncate|whitespace)-|hover:|focus:|disabled:|lg:|sm:|md:|:not\(|\[tabindex|sans-serif|serif$|^Bearer |^application\/|^image\/|^var\(--|^(row|column|center|flex-start|flex-end|space-between|space-around|contain|cover|stretch|repeat|nowrap|wrap|hidden|visible|absolute|relative|solid|dashed|dotted|none|auto|bold|normal|italic|uppercase|lowercase|capitalize)$/;
